...
const IntakeForm = ({ selectedPackage, setSelectedPackage }) => {
  const [form, setForm] = useState({
    companyName: '', website: '', decisionMaker: '', hours: '', otherBottleneck: '', email: '', phone: '',
  });
  ...
  const submit = (ev) => {
    ...
    const payload = { ...form, bottlenecks, package: selectedPackage, submittedAt: new Date().toISOString() };
    try {
      const prev = JSON.parse(localStorage.getItem('apex_leads') || '[]');
      localStorage.setItem('apex_leads', JSON.stringify([payload, ...prev]));
    } catch (_) {}
    setSubmitted(true);
    setTimeout(() => document.getElementById('intake')?.scrollIntoView({ behavior: 'smooth' }), 40);
  };

  if (submitted) {
    return (
      <section id="intake" className="scroll-mt-24 bg-[#F4F4F5] py-24 lg:py-32">
        <div className="container mx-auto px-6 max-w-2xl">
          <div className="bg-white rounded-[32px] border border-black/8 p-10 lg:p-14 text-center shadow-[0_20px_60px_rgba(14,14,18,0.08)]">
            ...
            <button
              ...
              className="mt-8 inline-flex items-center gap-2 bg-ink text-white rounded-full px-7 py-3.5 font-bold hover:bg-brand-600 transition-colors"
            >
              Back to top <ArrowRight size={18} className="rotate-[-45deg]" />
            </button>
          </div>
        </div>
      </section>
    ...
    <section id="intake" className="scroll-mt-24 bg-[#F4F4F5] py-24 lg:py-32">
      <div className="container mx-auto px-6 max-w-3xl">
        <div className="text-center mb-10">
          <span className="text-brand-600 font-bold tracking-[0.18em] text-xs uppercase">Book your consultation</span>
          <h2 className="font-display font-bold text-ink text-4xl lg:text-5xl mt-4 leading-tight">Tell us about your bottleneck.</h2>
          <p className="text-ink/60 text-lg mt-5 leading-relaxed max-w-xl mx-auto">
            This is the first step — not busywork. We review every submission before scheduling a call.
          </p>
        </div>

        <form onSubmit={submit} className="bg-white rounded-[32px] border border-black/8 p-7 sm:p-10 shadow-[0_20px_60px_rgba(14,14,18,0.08)] space-y-7" noValidate>
              ...
              {BOTTLENECKS.map((b) => {
                ...
                return (
                  <label key={b} className={`flex items-center gap-3 rounded-xl border px-4 py-3.5 cursor-pointer transition-colors ${active ? 'border-brand-600 bg-brand-50' : 'border-black/12 hover:border-black/25'}`}>
                    <Checkbox checked={active} onCheckedChange={() => toggle(b)} className="w-5 h-5 border-ink/25 data-[state=checked]:bg-brand-600 data-[state=checked]:border-brand-600" />
